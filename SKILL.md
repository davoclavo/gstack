...
The browse CLI is the TypeScript file at `browse/src/cli.ts` in this skill's directory.
Run all browse commands with `bun run <this-skill-dir>/browse/src/cli.ts <command> <args>`.

On first use, if the command fails with missing modules, run `bun install` in this skill's directory (where `package.json` lives).
...
```bash
B="bun run <this-skill-dir>/browse/src/cli.ts"
